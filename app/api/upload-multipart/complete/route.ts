...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('[Complete Upload] Raw parts received:', JSON.stringify(partsRaw));

        // Ensure parts are sorted by PartNumber and have correct shape
        const parts = partsRaw
            .map((p: any) => {
                // R2/S3 ETags usually come with quotes, we need to ensure we send them back exactly as expected
                // The AWS SDK might expect them with or without quotes depending on version
                // Let's try keeping them as-is first, but ensure they are strings
                return {
        ...
        console.log('[Complete Upload] Processed parts for S3:', JSON.stringify(parts));
        ...
        try {
            ...
            console.log('[Complete Upload] Success:', completeRes);
        } catch (s3Error) {
            console.error('[Complete Upload] S3 Error:', s3Error);
            return NextResponse.json({ error: 'Failed to complete S3 multipart upload' }, { status: 500 });
        }
